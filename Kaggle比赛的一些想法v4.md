...
在训练集中，计算similarity在0.1以下，标签1的占比
```python
# -*- coding: utf-8 -*-
...
# 读取数据，并分为特征和目标变量
df = pd.read_csv('F://Kaggle/train_with_similarity.csv')

# 计算similarity在0.1以下，标签1的占比
new_df = df[df['similarity'] <= 0.1]
total = new_df.shape[0]
sim_1 = new_df['is_duplicate'].tolist().count(1)
print(total)
print(sim_1)
print('Percent: %.4f%%'%(sim_1*100/total))
```
...
特征提取， 基于TF-IDF的Word2Vec的向量表示，并考虑是否添加问题相似度这个特征。
...
将训练集进行划分，分为训练部分和测试部分，使用XGBoost模型进行建模。
...
```
......
[295]	train-auc:0.999843
[296]	train-auc:0.999843
[297]	train-auc:0.999845
[298]	train-auc:0.999845
[299]	train-auc:0.999846
AUC: 0.7824
ACC: 0.8055
Recall: 0.6941
F1-score: 0.7250
